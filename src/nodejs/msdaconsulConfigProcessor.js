...
const consulAPI = "/v1/catalog/service/";
 ...
 * @constructor
 */
function msdaconsulConfigProcessor() {
}

msdaconsulConfigProcessor.prototype.setModuleDependencies = function (options) {
    logger.info("setModuleDependencies called");
    configTaskUtil = options.configTaskUtil;
};

msdaconsulConfigProcessor.prototype.WORKER_URI_PATH = "shared/iapp/processors/msdaconsulConfig";

msdaconsulConfigProcessor.prototype.onStart = function (success) {
    logger.fine("MSDA: OnStart, msdaconsulConfigProcessor.prototype.onStart");
 ...
 */
msdaconsulConfigProcessor.prototype.onPost = function (restOperation) {
    var configTaskState,
        blockState,
        oThis = this;
    logger.fine("MSDA: onPost, msdaconsulConfigProcessor.prototype.onPost");
    ...
    let signalIndex = global.msdaconsulOnPolling.findIndex(instance => instance.name === instanceName);
    if (signalIndex !== -1) {
        //Already has the instance, set the pool change signal if the pool changed
        blockInstance.bigipPoolChange =
            global.msdaconsulOnPolling[signalIndex].bigipPool !== inputPoolName;
    ...
    if (global.msdaconsulOnPolling.some(instance => instance.bigipPool === inputPoolName)) {
        logger.fine(
            "MSDA: onPost, " +
            instanceName +
        ...
        global.msdaconsulOnPolling.push(blockInstance);
        logger.fine(
            "MSDA onPost: " + instanceName + " set msdaconsulOnpolling signal: ",
            global.msdaconsulOnPolling
        );
    }
    ...
    (function schedule() {
        var pollRegistry = setTimeout(function () {
            ...
            if (global.msdaconsulOnPolling.some(instance => instance.name === instanceName)) {
                let signalIndex = global.msdaconsulOnPolling.findIndex(instance => instance.name === instanceName);
                if (global.msdaconsulOnPolling[signalIndex].state === "update") {
                    if (existingPollingLoop) {
                        ...
                        global.msdaconsulOnPolling[signalIndex].state = "polling";
                        logger.fine(
                            "MSDA: onPost/polling, " +
                            instanceName +
                            " update the signal.state into polling for new polling loop: ",
                            global.msdaconsulOnPolling[signalIndex]
                        );
                    }
                }
                        ...
                        mytmsh.executeCommand("tmsh -a list ltm pool " + inputPoolName)
                            .then(function (result) {
                                ...
                                logger.fine(
                                    "MSDA: onPost, " +
                                    instanceName +
                                    " GET the pool failed, adding an empty pool: ",
                                    inputPoolName
                                );
                                let inputEmptyPoolConfig = inputPoolName + ' monitor ' + inputMonitor + ' load-balancing-mode ' + inputPoolType + ' members none';
                                let commandCreatePool = 'tmsh -a create ltm pool ' + inputEmptyPoolConfig;
        ...
        if (
            ...
            let signalIndex = global.msdaconsulOnPolling.findIndex(
                (instance) => instance.name === instanceName
            );
            if (global.msdaconsulOnPolling[signalIndex].state === "polling") {
                logger.fine(
                    "MSDA: onPost, " +
        ...
        if (stopPolling) {
            ...
            if (
                ...
                if (
                    global.msdaconsulOnPolling[signalIndex].bigipPoolChange === true
                ) {
                    logger.fine(
                    "MSDA: onPost, " +
                        instanceName +
                    ...
                    const commandDeletePool =
                        "tmsh -a delete ltm pool " + inputPoolName;
                    mytmsh
 ...
 * @param restOperation - originating rest operation that triggered this processor
 */
msdaconsulConfigProcessor.prototype.onDelete = function (restOperation) {
    var configTaskState,
        blockState;
    var oThis = this;

    logger.fine("MSDA: onDelete, msdaconsulConfigProcessor.prototype.onDelete");
...
module.exports = msdaconsulConfigProcessor;
