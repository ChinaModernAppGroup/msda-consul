...
msdaconsulConfigProcessor.prototype.onStart = function (success) {
    ...
    try {
        fs.access(msdaconsulOnPollingSignal, fs.constants.F_OK, function (err) {
            if (err) {
                logger.fine("MSDAconsul audit OnStart, the polling signal is off. ", err.message);
            } else {
                logger.fine("MSDA consul audit onStart: ConfigProcessor started, clear the polling signal.");
                fs.unlinkSync(msdaconsulOnPollingSignal);
            }
        });
    } catch(err) {
        logger.fine("MSDAconsul: OnStart, hits error while check polling signal. ", err.message);
    }
...
msdaconsulConfigProcessor.prototype.onPost = function (restOperation) {
    ...
    try {
        logger.fine("MSDAconsul: onPost, will set the polling signal. ");
        fs.writeFile(msdaconsulOnPollingSignal, '');
    } catch (error) {
        logger.fine("MSDAconsul: onPost, hit error while set polling signal: ", error.message);
    }
